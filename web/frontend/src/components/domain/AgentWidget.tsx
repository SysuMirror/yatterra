...
function AssistantSession({ user }: { user: string }) {
  ...
  return <>
    ...
    {open && <section role="dialog" aria-modal="false" aria-label="AI 助手" data-assistant-ui data-onboarding-overlay className="fixed right-2 z-[var(--z-fab)] flex w-[min(420px,calc(100vw-16px))] flex-col overflow-hidden rounded-2xl border border-black/10 bg-white/95 shadow-2xl backdrop-blur-xl" style={{top:viewport.top + Math.max(8,viewport.height - Math.min(560,viewport.height-16) - 8),height:Math.min(560,viewport.height-16)}}>
