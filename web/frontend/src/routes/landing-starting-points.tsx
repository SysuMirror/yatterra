...
export function StartingPointDiagram({ kind, paused = false }: { kind: string; paused?: boolean }) {
  const reduce = useReducedMotion()
  const still = paused || Boolean(reduce)
  const deviceFlow = still ? { cx: 312, cy: 108, opacity: .25 } : { cx: [214, 312], cy: [108, 108], opacity: [0, 1, 0] }
  const deviceFlowTransition = still ? { duration: .2 } : { duration: 2.2, repeat: Infinity, ease: 'linear' as const }
  if (kind === 'device') return <svg className="terra-explorer-art" viewBox="0 0 560 230" fill="none" role="img" aria-label="一台设备接入网络并承接任务的示意图">
    ...
    <path d="M210 108H316" stroke="#67e8d0" strokeOpacity=".6" strokeDasharray="5 7" /><motion.circle initial={false} r="4" fill="#67e8d0" animate={deviceFlow} transition={deviceFlowTransition} />
    ...
    <path d="M416 108H510" stroke="#c4b5fd" strokeOpacity=".6" strokeDasharray="5 7" /><motion.circle initial={false} r="4" fill="#c4b5fd" animate={still ? { cx: 506, cy: 108, opacity: .25 } : { cx: [420, 506], cy: [108, 108], opacity: [0, 1, 0] }} transition={still ? { duration: .2 } : { duration: 2.2, delay: .8, repeat: Infinity, ease: 'linear' }} />
  ...
  if (kind === 'network') return <svg className="terra-explorer-art" viewBox="0 0 560 230" fill="none" role="img" aria-label="一组节点通过调度器协作完成任务的示意图">
    ...
    <motion.circle initial={false} r="4" fill="#f6c889" animate={still ? { cx: 280, cy: 115, opacity: .25 } : { cx: [132, 264, 296, 428], cy: [48, 108, 122, 182], opacity: [0, 1, 1, 0] }} transition={still ? { duration: .2 } : { duration: 3, repeat: Infinity, ease: 'easeInOut' }} />
  </svg>
